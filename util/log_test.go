...
func TestGenLogId(t *testing.T) {
	...
	//16位且递增
	if len(Int2Str(id1)) != wantIdLen {
		t.Errorf("GenLogId 位数 = %d, 期望 %d", len(Int2Str(id1)), wantIdLen)
	}
...
func TestGenReqId(t *testing.T) {
	...
	//与 GenLogId/GenId 同源，均为16位时间序ID
	if got := len(Int2Str(id1)); got != wantIdLen {
		t.Errorf("GenReqId 位数 = %d (%s), 期望 %d", got, Int2Str(id1), wantIdLen)
	}
