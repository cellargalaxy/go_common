...
func TestJsonRoundTrip(t *testing.T) {
	...
	text := JsonStruct2Str(list)
	var got []jsonDemo
	if err := JsonStr2Struct(text, &got); err != nil {
...
// 校验实际序列化文本，而非仅"能转回来"
func TestJsonStruct2Str(t *testing.T) {
	got := JsonStruct2Str(jsonDemo{Id: 1, Name: "n", Skip: "x"})
	//json标签必须生效
	...
	for in, want := range cases {
		if got := JsonStruct2Str(in); got != want {
			t.Errorf("JsonStruct2Str(%v) = %q, 期望 %q", in, got, want)
		}
	}
	//切片与map
	if got := JsonStruct2Str([]int{1, 2}); got != "[1,2]" {
		t.Errorf("切片 = %q", got)
	}
	if got := JsonStruct2Str(map[string]int{"a": 1}); got != `{"a":1}` {
		t.Errorf("map = %q", got)
	}
	//中文不应被转义成\u
	if got := JsonStruct2Str(map[string]string{"k": "中文"}); !strings.Contains(got, "中文") {
		t.Errorf("中文被转义: %s", got)
	}
	//Data 版本与 String 版本必须一致
	if string(JsonStruct2Data(list1())) != JsonStruct2Str(list1()) {
...
func TestJsonIndent(t *testing.T) {
	got := JsonStruct2StrIndent(jsonDemo{Id: 1, Name: "a"})
	...
	//缩进版与紧凑版语义应等价
	var a, b jsonDemo
	if err := JsonStr2Struct(got, &a); err != nil {
		t.Fatalf("%+v", err)
	}
	if err := JsonStr2Struct(JsonStruct2Str(jsonDemo{Id: 1, Name: "a"}), &b); err != nil {
		t.Fatalf("%+v", err)
	}
	if a != b {
	...
	//Data 与 String 版本一致
	if string(JsonStruct2DataIndent(jsonDemo{Id: 1})) != JsonStruct2StrIndent(jsonDemo{Id: 1}) {
...
func TestJsonError(t *testing.T) {
	...
	for _, bad := range []string{"", "{", "not json", `{"id":"字符串给了int字段"}`} {
		if err := JsonStr2Struct(bad, &got); err == nil {
			t.Errorf("JsonStr2Struct(%q) 应返回error", bad)
		}
	}
	//Data 版本同样报错
	...
	if got := JsonStruct2Str(func() {}); got != "" {
		t.Errorf("不可序列化类型 = %q, 期望空串", got)
	}
	if got := JsonStruct2Str(math.NaN()); got != "" {
		t.Errorf("NaN = %q, 期望空串", got)
	}
	//传入非指针时应报错而不是静默成功
	if err := JsonStr2Struct(`{"id":1}`, got); err == nil {
