...
type fakeConfigHandler struct {
	filePath   string
	defaultCfg string
	parsed     atomic.Value //最近一次 Parse 收到的内容
...
func TestConfigServiceLoadConfigFirstTime(t *testing.T) {
	...
	got, err := ReadFile2Str(ctx, h.filePath, "")
	if err != nil {
		t.Fatalf("%+v", err)
	}
	if got != "默认配置内容" {
	...
	//内存中的配置也须就绪
	if got = service.text; got != "默认配置内容" {
		t.Errorf("缓存配置 = %q", got)
	}
	//handler 须被回调解析
	if h.parseCount.Load() != 1 {
		t.Errorf("Parse 调用次数 = %d, 期望 1", h.parseCount.Load())
	}
	if h.lastParsed() != "默认配置内容" {
		t.Errorf("Parse 收到 = %q", h.lastParsed())
	}
}
...
func TestConfigServiceLoadConfigNoChange(t *testing.T) {
	...
	if h.parseCount.Load() != 1 {
		t.Fatalf("首次 Parse 次数 = %d", h.parseCount.Load())
	}
	//重复加载：内容一致，不应再次解析
	for i := 0; i < 3; i++ {
		if err := service.LoadConfig(ctx); err != nil {
	...
	if h.lastParsed() != "cfg-v2" {
		t.Errorf("Parse 收到 = %q, 期望 cfg-v2", h.lastParsed())
	}
	if got := service.text; got != "cfg-v2" {
		t.Errorf("缓存配置 = %q, 期望 cfg-v2", got)
	}
}

// Parse 报错时不得把配置标记为已生效，否则错误配置会被固化
func TestConfigServiceParseError(t *testing.T) {
	...
	//解析失败不应更新内存配置
	if got := service.text; got != "" {
		t.Errorf("解析失败后缓存配置 = %q, 期望仍为空", got)
	}
	//修复后重新加载须成功
	h.setParseErr(nil)
	if err := service.LoadConfig(ctx); err != nil {
	...
	if got := service.text; got != "坏配置" {
		t.Errorf("修复后缓存配置 = %q", got)
	}
}
...
func TestConfigServiceStart(t *testing.T) {
	...
	//首次加载须已完成
	if got := service.text; got != "启动配置" {
		t.Errorf("Start 后缓存配置 = %q", got)
	}
	if service.pool == nil {
...
func TestConfigServiceFlushCtxNotLeak(t *testing.T) {
	...
	if got := service.text; got != "cfg" {
		t.Errorf("运行中缓存配置 = %q", got)
	}
	if service.pool.IsClose(ctx) {
		t.Errorf("运行中池被意外关闭")
	}
